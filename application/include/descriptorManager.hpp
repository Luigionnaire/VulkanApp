...
public:
	...
	void createDescriptorSets(const std::array<VkImageView, 4>& imageViews, const std::array<VkSampler, 4>& sampler) {
		std::vector<VkDescriptorSetLayout> layouts(MAX_FRAMES_IN_FLIGHT, m_descriptorSetLayout);
		VkDescriptorSetAllocateInfo allocInfo{};
		allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
		allocInfo.descriptorPool = m_descriptorPool;
		...
		for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
			...
			for (size_t j = 0; j < 4; ++j) {
				...
				descriptorWrites[j + 1].dstBinding = static_cast<uint32_t>(j + 1); // Bindings 1 to 4
...
private:
	...
	void createDescriptorSetLayout() {
		VkDescriptorSetLayoutBinding uboLayoutBinding{}; // Binding 0: UBO
		uboLayoutBinding.binding = 0;
		uboLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
		uboLayoutBinding.descriptorCount = 1;
		uboLayoutBinding.stageFlags = VK_SHADER_STAGE_VERTEX_BIT;
		...
		// Bindings 1–4: Combined image samplers
